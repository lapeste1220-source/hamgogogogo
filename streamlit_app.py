...
def apply_custom_theme():
    ...
    :root{
        --navy:#16335c;          /* 메인 네이비 */
        --navy-deep:#0f2444;     /* 짙은 네이비 */
        --accent:#2f6fd0;        /* 포인트 블루 */
        --accent-soft:#e8f0fb;   /* 연한 블루 배경 */
        --ink:#1c2733;           /* 본문 텍스트 */
        --muted:#6b7785;         /* 보조 텍스트 */
        --line:#e3e8ef;          /* 구분선 */
        --card-shadow:0 4px 20px rgba(22,51,92,.07);

        /* 추천 구분 색상 */
        --c-high:#d98324;        --c-high-bg:#fdf3e6;   /* 상향(도전) */
        --c-mid:#2f6fd0;         --c-mid-bg:#e8f0fb;    /* 적정 */
        --c-safe:#2e8b6f;        --c-safe-bg:#e6f5ef;   /* 안전 */

        /* 최저 상태 색상 */
        --ok:#2e8b6f;            --ok-bg:#e6f5ef;
        --warn:#c0563f;          --warn-bg:#fbeae6;
    }
    ...
    [data-testid="stSidebar"] .stRadio label:hover{
        background:rgba(255,255,255,.08); padding-left:.6rem;
    }
    ...
    .stButton > button{
        ...
        box-shadow:0 2px 8px rgba(47,111,208,.25);
    }
    .stButton > button:hover{
        background:var(--navy); transform:translateY(-2px);
        box-shadow:0 6px 18px rgba(22,51,92,.28);
    }

    /* 입력 위젯 */
    [data-baseweb="input"], .stNumberInput input, .stTextInput input{
    ...
    [data-testid="stMetric"]{
        ...
        box-shadow:var(--card-shadow); animation:fadeIn .6s ease both;
    }
    ...
    .hch-hero{
        ...
        color:#fff; border-radius:18px; padding:1.6rem 2rem; margin-bottom:1.4rem;
        box-shadow:0 8px 30px rgba(22,51,92,.22); animation:fadeInUp .6s ease both;
    }
    ...
    .hch-section{
        ...
        font-size:1.25rem; font-weight:700; color:var(--navy-deep);
        margin:1.4rem 0 .8rem; padding-left:.7rem;
        border-left:5px solid var(--accent); animation:fadeInUp .5s ease both;
    }
    ...
    .rec-badge{
        display:inline-block; font-size:.74rem; font-weight:700; letter-spacing:.2px;
        padding:.18rem .6rem; border-radius:6px; margin-bottom:.55rem;
    }
    ...
    .diff-up{ color:var(--c-high); }     /* 도전 */
    .diff-mid{ color:var(--c-mid); }
    .diff-down{ color:var(--c-safe); }   /* 여유 */

    /* 추천 카드 미니 게이지 (나 vs 합격선) */
    ...
    .gauge-cut.high{ background:var(--c-high); }
    .gauge-cut.mid{  background:var(--c-mid); }
    .gauge-cut.safe{ background:var(--c-safe); }
    .gauge-me{ position:absolute; top:-1px; width:13px; height:13px; border-radius:50%; transform:translateX(-50%);
               background:#fff; border:3px solid var(--navy); box-shadow:0 1px 4px rgba(0,0,0,.2); z-index:2; }
    ...
    .min-card{
        ...
        padding:1rem 1.1rem; box-shadow:var(--card-shadow); animation:fadeInUp .5s ease both;
        transition:transform .18s ease, box-shadow .2s ease;
    }
    .min-card:hover{ transform:translateY(-3px); box-shadow:0 10px 26px rgba(22,51,92,.14); }
    .min-card.ok{ border-top:4px solid var(--ok); }
    .min-card.warn{ border-top:4px solid var(--warn); }
    ...
    .min-tag{ display:inline-block; font-size:.74rem; color:var(--muted);
              background:#f1f4f8; border-radius:6px; padding:.12rem .5rem; margin:.45rem .3rem 0 0; }
    .min-reason{ font-size:.8rem; margin-top:.55rem; padding:.5rem .65rem; border-radius:8px; line-height:1.45; }
    .min-reason.ok{   background:var(--ok-bg);   color:var(--ok); }
    .min-reason.warn{ background:var(--warn-bg); color:var(--warn); }
    .min-rule{ font-size:.8rem; color:var(--muted); margin-top:.55rem; padding-top:.5rem;
               border-top:1px dashed var(--line); line-height:1.45; }
    ...
    .kpi-card{
        background:#fff; border:1px solid var(--line); border-radius:14px;
        padding:1rem 1.15rem; box-shadow:var(--card-shadow);
    ...
    .kpi-card.k-ok::before{ background:var(--c-safe); }
    .kpi-card.k-rate::before{ background:var(--c-high); }
    .kpi-card.k-grade::before{ background:var(--navy); }
    ...
    .filter-wrap{
        background:#fff; border:1px solid var(--line); border-radius:14px;
        padding:.4rem 1.1rem 1rem; box-shadow:var(--card-shadow); margin-bottom:.4rem;
    }
    </style>
...
def section(title, icon="●"):
    st.markdown(f'<div class="hch-section">{icon} {title}</div>', unsafe_allow_html=True)
...
def _hch_altair_theme():
    return {
        "config": {
            ...
            "range": {"category": ["#16335c", "#2f6fd0", "#5b9bd5",
                                   "#f29f3d", "#7a8aa0", "#9ec6f0"]},
        }
    }
...
def explain_minimum_rule(rule_text, grades):
    ...
    지원 규칙: 'N등급 이내' / '(중) N개 영역 합 M 이내' / '각 N등급'
    """
    ...
    if m_sum:
        ...
        return ok, f"상위 {n}개 영역 합 {pick}={s} (기준 {limit} 이내) → {verdict}"
    ...
    if m_each:
        ...
        verdict = "충족" if ok else "미충족"
        return ok, f"전 영역 {limit}등급 이내 · 내 최저 {worst}등급 → {verdict}"
    ...
    if m_each2:
        ...
        ok = all(g <= limit for g in nums)
        verdict = "충족" if ok else "미충족"
        return ok, f"각 영역 {limit}등급 기준 · 내 최저 {worst}등급 → {verdict}"
...
def render_rec_cards(rec: pd.DataFrame, score_col: str, diff_col: str,
    ...
    """
    rec: 추천 결과 DataFrame (추천구분 컬럼 필요)
    score_col: 카드 하단 수치 컬럼 (합격평균내신 / 정시평균백분위)
    diff_col: 차이 컬럼 (score - 내값)
    lower_is_harder: True=내신(작을수록 도전) / False=백분위(클수록 도전)
    """
    label_meta = {
    ...
    for _, r in rec.iterrows():
        ...
        gauge_html = ""
        if sval is not None:
            myv = sval - d  # score - (score - 내값) = 내값
            me_pos = _pos(myv)
            cut_pos = _pos(sval)
            f_lo, f_hi = sorted([me_pos, cut_pos])
            gauge_html = (
                f'<div class="gauge">'
                f'<div class="gauge-track"></div>'
        ...
        <div class="rec-card {cls}">
            <span class="rec-badge {badge_cls}">{badge_txt}</span>
            <div class="rec-univ">{univ}</div>
            <div class="rec-major">{major}</div>
            {meta_html}
            {gauge_html}
            <div class="rec-metric">
                <div><span class="l">{score_label}</span><br><span class="v">{sval_txt}</span></div>
                <div class="rec-diff {diff_cls}">{diff_txt}</div>
            </div>
        </div>
        """)
...
def render_choejeo_status(df: pd.DataFrame, status_col: str, reason_col=None,
    ...
    for _, r in d.iterrows():
        ...
        cards.append(f"""
        <div class="min-card {cls}">
            <span class="min-status {s_cls}">{s_txt}</span>
            <div class="min-univ">{univ}</div>
            <div class="min-major">{major}</div>
            <div>{tags_html}</div>
            {reason_html}
            {rule_html}
        </div>
    ...
    st.markdown(f'<div class="min-grid">{"".join(cards)}</div>', unsafe_allow_html=True)
...
def render_kpi_cards(total_apply, total_admit, avg_grade):
    ...
    avg_txt = f"{avg_grade:.2f}" if avg_grade == avg_grade else "-"  # NaN 방어
    html = f"""
    <div class="kpi-grid">
        <div class="kpi-card">
            <div class="kpi-label">전체 지원</div>
            <div class="kpi-value">{total_apply:,}<span class="kpi-unit">건</span></div>
            <div class="kpi-sub">검색 조건 기준</div>
        </div>
        <div class="kpi-card k-ok">
            <div class="kpi-label">합격</div>
            <div class="kpi-value">{total_admit:,}<span class="kpi-unit">건</span></div>
            <div class="kpi-sub">최종 합격 처리 기준</div>
        </div>
        <div class="kpi-card k-rate">
            <div class="kpi-label">합격률</div>
            <div class="kpi-value">{rate:.1f}<span class="kpi-unit">%</span></div>
            <div class="kpi-sub">합격 / 전체 지원</div>
        </div>
        <div class="kpi-card k-grade">
            <div class="kpi-label">합격자 평균 내신</div>
            <div class="kpi-value">{avg_txt}<span class="kpi-unit">등급</span></div>
            <div class="kpi-sub">대표등급 평균</div>
        </div>
    </div>
    """
...
def view_grade_analysis():
    ...
    base = filtered.assign(
        전형분류=lambda d: d[vt_col].astype(str).str.extract("(교과|종합|농어촌)", expand=False).fillna("기타")
    )
...
def view_recommend():
    ...
    with tab_su:
        ...
        if rec.empty:
            ...
            render_rec_cards(
                rec, score_col="합격평균내신", diff_col="내신차이(합-입)",
    ...
    with tab_je:
        ...
        if recj.empty:
            ...
            render_rec_cards(
                recj, score_col="정시평균백분위", diff_col="백분위차이(합-입)",
...
# =========================================
#   뷰 3 : 최저 기준으로 대학 찾기 (2027최저모음.csv만)
# =========================================
def view_choejeo():
    ...
    if st.button("검색", type="primary"):
        ...
        # 🎨 ② 충족/미충족 ✅/⚠️ 상태 카드 + 판정 근거
        render_choejeo_status(
            view_df, status_col="최저충족가능", reason_col="판정근거",
